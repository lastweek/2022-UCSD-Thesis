...
%\ulinebfpara{Data migration.}
%We use two ZCU106 \sysboard{}s to evaluate the performance of inter-\MN\ data migration.
%Migrating 1\GB\ data takes around 1.3 seconds.
%This speed is somewhat lower than the 10\Gbps\ board port bandwidth, because of the additional tasks required to clear and set up metadata (\eg, PTEs) at both boards, which involves the slow path.

%As \sysboard-to-\sysboard\ communication is fast (with just the link layer), 
%data migration reaches the 10\Gbps, (sending 1\GB\ data takes around 1.3 seconds).
...
%It adopts stall-and-forward buffer to calculate the checksum in UDP header for every out-going packet.
...
%(process for RDMA and collection for \sys)
...
%\zhiyuan{Add number of concurrent connections here? Tonic: 1024 Connections, no swap; do not include RDMA functions. StRoM: 500 QP. no swap.}
