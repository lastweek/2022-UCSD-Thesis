...
%\boldpara{Address translation flow.}
%When a \sysread/\syswrite\ request arrives and passes the network stack,
%\sys\ looks up its TLB based on the VA and PID.
%If it is a TLB hit, 
...
\if 0
\boldpara{Idempotence.}
In \sys, most types of requests like \sysread\ and \syswrite\ are idempotent and can be retried multiple times with the same result.
\sys\ also includes a few types of non-idempotent requests such as atomic increment.
%Non-idempotent requests such as atomic or synchronization requests (\eg, \texttt{rlock}) should run \textit{exactly once} even when retried.
...
%To ensure that the ring buffer does not overflow, \CN\ do not issue any new non-idempotent requests if it has not get the response of $K$ recent non-idempotent requests.
