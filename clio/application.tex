...
The utility simply reads a photo from \MN\ using \sysread, compresses/decompresses it,
and writes it back to the other array using \syswrite.
...
data with an \syswrite. It then calculates the hash and the fingerprint of the key. 
...
chain at a time using \sysread. It then compares the fingerprint of the key to the seven entries in the slot. If there is no match, it
...
entry with an \sysread. It then compares the full key and returns
